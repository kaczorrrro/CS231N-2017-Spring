...
def affine_forward(x, w, b):
    ...
    num_elems = x.shape[0]
    #input_elem_dim = reduce(x.shape, lambda x,y:x*y)/num_elems
    x_temp = x.reshape((num_elems, -1))
    out1 = x_temp@w

    out = out1+ np.expand_dims(b, 0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mean = np.mean(x, 0)
        mean_of_sqr = np.mean(x**2, 0)
        var = np.var(x, 0)
        b = 1/(np.sqrt(var)+eps)
        a = (x-mean)*b
        out = a*gamma+beta
        running_mean = momentum*running_mean + mean*(1-momentum)
        running_var = momentum*running_var + var*(1-momentum)

        cache['x']              = x
        cache['mean']           = mean
        cache['mean_of_sqr']    = mean_of_sqr
        cache['var']            = var
        cache['a']              = a
        cache['b']              = b
        cache['gamma']          = gamma
...
def batchnorm_backward(dout, cache):
    ...
    x = cache['x']          
    mean = cache['mean']       
    mean_of_sqr = cache['mean_of_sqr']
    var = cache['var']        
    a = cache['a']          
    b = cache['b']          
    gamma = cache['gamma']      
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout*a, axis=0)
    

    #dx += 2/N*( (x[0,0]-mean)*dout[0,0] + (x[0,0]-mean)*dout[1,0])
    #dx += -1/N*np.sum(b**(3)*(x-mean)@dout.T, axis=1).reshape((-1,1)) #1/std(x)
    #dx += b*dout*gamma -1/N*b*np.sum(dout)*gamma -1/N*np.sum(b**(3)*(x-mean)@(dout*(x-mean)).T, axis=1).reshape((-1,1))*gamma #full unoptimized working version
    #dx += b*gamma*(dout -np.mean(dout) -b**2*np.mean((x-mean)@(dout*(x-mean)).T, axis=1).reshape((-1,1)))
    dx += b*gamma*(dout -np.mean(dout,0) -b*b*(x-mean)*np.mean(dout*(x-mean),0))
    # dx[0,0] = b*dout[0,0] + -1/N*(
    #      x[0,0]*b**3*(x[0,0]-mean)*dout[0,0]
    #     +x[1,0]*b**3*(x[0,0]-mean)*dout[1,0]
    #     +x[2,0]*b**3*(x[0,0]-mean)*dout[2,0])
...
####################################
    ...
    for f in range(F):
        for n in range(N):
            for _x in range(Wout):
                x_in_start = _x*stride
                for y in range(Hout):
                    y_in_start = y*stride
                    out[n, f, y, _x] = np.sum(padded[n,:,y_in_start:y_in_start+HH,x_in_start:x_in_start+WW]*w[f,:,:,:])
                    out[n,f,y,_x]+=b[f]
...
def magic_shuffle(tensor):
    return np.swapaxes(np.flip(np.flip(tensor, 2),3),0,1)
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
        for f in range(F):
            for c in range(C): 
                dw[f,c,:,:] += conv_forward_naive(x[n:n+1,c:c+1,:,:], dout[n:n+1,f:f+1,:,:],np.zeros(dout.shape[0]), conv_param)[0].squeeze()
    db = dout.sum(0).sum(1).sum(1)

    dx = np.zeros_like(x)
    dx += conv_forward_naive(dout, magic_shuffle(w), np.zeros(dx.shape[1]),  {'stride':1, 'pad':HH-1-conv_param['pad']})[0]
...
def max_pool_forward_naive(x, pool_param):
    ...
    Hout = H//stride
    Wout = W//stride
    out_shape = (N,C, Hout, Wout)
    out = np.zeros(out_shape)
    in_mask = np.zeros(x.shape, dtype=bool)
    for n in range(N):
        for c in range(C):
            for h in range(0, Hout):
                ...
                for w in range(0, Wout):
                    w_in_start = w*stride
                    pool_region = x[n,c, h_in_start:h_in_start+pool_height, w_in_start:w_in_start+pool_width]
                    argmax = np.argmax(pool_region)
                    index = np.unravel_index(argmax, (pool_height, pool_width))
                    out[n,c,h,w] = pool_region[index]
                    in_mask[n,c,h_in_start+index[0],w_in_start+index[1]] = True


    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, in_mask, pool_param)
    return out, cache
    ...
    for n in range(N):
        for c in range(C):
            for h in range(H):
                for w in range(W):
                    if in_mask[n,c,h,w]:
                        dx[n,c,h,w] = dout[n,c,h//pool_height,w//pool_width]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    (N, C, H, W) = x.shape
    out_reshaped = x.swapaxes(1,3).reshape((-1, C))
    out, cache = batchnorm_forward(out_reshaped, gamma, beta, bn_param)
